...
from database import create_db, insert_categories, insert_videos, insert_channels
...
if __name__ == "__main__":
    ...
    print(f"\n{len(categories_list)} categorias coletadas.")
    ...
    all_videos = []
    for category in categories_list:
        print(f"\nColetando videos da categoria {category["snippet"]["title"]}.")
        try:
            ...
            print(f"\nA categoria '{category["snippet"]["title"]}' (id {category["id"]}) foi pulada pois não existem videos em alta nela.")
    ...
    print("\nColetando estatisticas dos canais:")
    channels_list = []
    for i in range(0,len(unique_channels),50):
        print(f"Coletando do lote de canais de {i} a {i + 50}")
    ...
    print("\nInserindo dados na tabela de videos...")

    insert_videos(con = con, videos= all_videos)

    print("\nInserindo dados na tabela de canais...")
    ...
    print(f"\nPipeline concluído: {len(all_videos)} vídeos, {len(channels_list)} canais e {len(categories_list)} categorias inseridos. Duração: {duration}.")
